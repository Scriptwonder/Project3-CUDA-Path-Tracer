    ...
    )
    {
        ...
        if (path_index < num_paths)
        {
            ...
            else
            {
                ...
                intersection.pos = intersect_point;
            }
        }
    }
                ...
                if (material.emittance > 0.0f || material.emissiveTexture.index >= 0) {
                    if (material.gltf) {
                        ...
                        if (glm::length(EmissiveColor) > 0.0f) {
                            ...
                            scatterRayGLTF(pathSegment, getPointOnRay(pathSegment.ray, intersection.t), intersection.surfaceNormal, intersection.uv, intersection.tangent, material, rng, cudaTex, dev_texData);
    ...
    __global__ void atrousFilter(const glm::ivec2 resolution, const glm::vec3* in, glm::vec3* out, const GBufferPixel* gBuffer,
        ...
        for (int dy = -2; dy <= 2; ++dy) {
            for (int dx = -2; dx <= 2; ++dx) {
                ...
                const float c_w = min(exp(-dist2 / c_phi), 1.f);
                ...
                const int kernel_idx = min(abs(dx), abs(dy));
                sum += ctemp * weight * kernel[kernel_idx];
                cum_w += weight * kernel[kernel_idx];
            }
        }
        out[idx] = sum / cum_w;
    }
